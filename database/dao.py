...
class DAO:
    ...
    @staticmethod
    def get_artist_min_album(n_alb):
    ...
    def get_edges():
        ...
        query = (""" SELECT a1.id as id1, a2.id as id2, COUNT( DISTINCT t1.genre_id) as peso
                     ...
                     WHERE a1.id = al1.artist_id AND a2.id = al2.artist_id AND al1.id = t1.album_id 
                     AND al2.id = t2.album_id AND t1.genre_id = t2.genre_id AND a1.id < a2.id
        ...
        try:
            cursor.execute(query)
            for row in cursor:
                connessione = (row['id1'], row['id2'], row['peso'])
